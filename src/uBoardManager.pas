...
begin
  ...
  if Bmp <> nil then TargetImg.Bitmap.Assign(Bmp);
end;
...
begin
  ...
  if FActiveBoardIdx >= Length(FAllCoords)
  then SetLength(FAllCoords, FActiveBoardIdx + 1);
...
begin
  Result := (FActiveBoardIdx < Length(FAllCoords)) and (Length(FAllCoords[FActiveBoardIdx]) > 0);
end;
